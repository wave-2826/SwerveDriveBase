...
void SwerveMath::Calculate(double x, double y, double z, double angle, SwerveMathOutput output[4])
{
	if(angle != -999.0)
	{
		angle = angle * PI / 180;
    ...
    output[0].speed = wSpeed1; output[0].angle = wAngle1;
    output[1].speed = wSpeed2; output[1].angle = wAngle2;
    output[2].speed = wSpeed3; output[2].angle = wAngle3;
    output[3].speed = wSpeed4; output[3].angle = wAngle4;
}
